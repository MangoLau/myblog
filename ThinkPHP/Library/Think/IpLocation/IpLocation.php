...
* @modified by david  blog:blog.iyi.cn bbs:www.fenghuanghua.com
	...
	$o = new IpLocation();
	$x = $o->getlocation('121.203.31.168');
	var_dump($x);
*/
class IpLocation {
    /**
    ...
    function getlocation($ip, $separator = 0) {
    	$cache = in_array($config['langtype'], array(1)) ? true : false;
    	if($cache && (($location = ocache::redisIp()->get( ip2long($ip) ) ) != false ) ){
    		return $location;
    	}
        if (!$this->fp) return null;            // 如果数据文件没有被正确打开，则直接返回空
    	...
    	$cache && ocache::redisIp()->set(ip2long($ip), $location);
        return $location;
    }
    ...
    function IpLocation() {
        $this->fp = 0;
        if (($this->fp = fopen(dirname(__FILE__) . '/QQWry.dat', 'rb')) !== false) {
